"""
Script for counting the shared vertices.
...
if __name__ == '__main__':
    ...
    vertex_counts = {i: 0 for i in range(len(data['vertices']))}
    ...
    vertex_counts_sorted = dict(sorted(vertex_counts.items(), key=lambda item: item[1], reverse=True))

    nr_shared = 0
    with inpath.parent.joinpath(inpath.stem).with_suffix(".csv").open("w") as f:
        csv_writer = csv.writer(f)
        csv_writer.writerow(["Vertex", "Count"])
        for vertex, count in vertex_counts_sorted.items():
            if count > 1:
                nr_shared += 1
                csv_writer.writerow([vertex, count])
