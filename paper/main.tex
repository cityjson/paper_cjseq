...
% @Hugo we should find a better way to share a link to the data. We should also add it as a footnote?
%
...
% @Hugo I think this link is unecessary. Or at least let\s make it a footnote.
\begin{table*}
  \centering
...
For example, CityJSONSeq is used by \emph{cjdb} (a tool for storing CityJSON datasets in a relational database) so as to store independently one feature per row in the database.
Additionally, in order to facilitate pagination, CityJSONSeq is the return format of the 3DBAG API. 
% @Hugo: I added these examples, do we need to reference the cjdb paper?
%
