...
We elaborate on the engineering decisions that were taken to develop CityJSONSeq, we present the open-source software we have developed to convert to/from CityJSONSeq, and discuss different aspects of the resulting CityJSONSeq, \eg\ size, usability, etc.

We think that CityJSONSeq is a better format than CityJSON
CityJSONSeq is at about 10\% more compacter, takes an order of magnitude less time to process and uses significantly less memory (for some operations, not all).
...
Because more and more datasets from cities are very large (it is not uncommon to have CityGML-GML files over 2GB), this made the processing of CityJSON difficult.
...
We have implemented and released an open-source program to convert between in either direction between CityJSON and CityJSONSeq objects; we describe it in Section~\ref{sec:experiments}.
...
Finally, we will also present the open-source software that are currently being developed to process streams of CityJSON datasets.
