...
\begin{figure*}
  ...
  \end{subfigure}
  \caption{Compression factor of CityJSONSeq files for different synthetic datasets. \textbf{(a)} Based on the number of buildings; the buildings are stored as simple cuboid that are randomly generated. \textbf{(b)} Based on the percentage of shared vertices. The same cuboid buildings are used, but we locate them adjacent to each others to create shared vertices.}%
