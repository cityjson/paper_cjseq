...
\begin{table*}
  ...
  \begin{tabular}
    ...
    \textbf{Helsinki}       && \qty{77231} bldgs   &         && \qty{572}{\mega\byte} & \qty{412}{\mega\byte} & 39\%  &&   \num{3038576} &    \num{2202} &  0.0\% \\
...
First observe that---contrary to intuition---the size of a dataset serialised as a CityJSONSeq file is around 12\% more compact than serialised as a CityJSON file, and in the case of \textbf{Helsinki} it is 39\% compacter.
...
As an experiment, we have created around 125 synthetic CityJSON datasets containing buildings, and each building is represented as a simple cube randomly generated; Figure~\ref{fig:synthetic_compression} shows the results.
\begin{figure}
  \centering
  \includegraphics[width=\linewidth]{figs/synthetic_compression.pdf}
  \caption{Compression factor of CityJSONSeq for different synthetic datasets containing only but simple cuboid buildings.}%
\label{fig:synthetic_compression}
\end{figure}
Notice that as the filesize grows, the compression factor ($\frac{size(CityJSON) - size(CityJSONSeq)}{CityJSON}$) grows.
The smallest file contains only but 526 buildings and its compression factor is -2\%, while the largest file has \num{3960105} buildings, and a compression factor of 18\%.
