...
The main idea is to decompose a CityJSON file into its individual features (eg each Building, each Bridge, etc.) and create several JSON objects of a newly defined type: \texttt{CityJSONFeature}.
...
We consider CityJSONSeq to be a better format than CityJSON, since it is at about 10\% more compact, takes an order of magnitude less time to process and uses significantly less memory for certain operations.
...
\emph{XLinks} are links links between elements in a file (similar to \emph{pointers} in a computer program), a concrete example in a CityGML file is a cube that lists the geometries of 5 of its surfaces, but its 6th surface is simply a link to another surface somewhere else in the file (which belongs to another building for instance).
If in the stream the linked surface has not appeared yet, the receiver cannot process the cube and needs to wait for that specific surface to appear.
...
However, it is always possible to resolve the \emph{XLinks} before streaming a dataset, but this means that its size is increased, and that converting back to the original file will not be possible.
...
The fact that many indices are used for geometries (and for textures) means that if large numbers are used then the size of the file will grow.
